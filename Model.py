...
class UNet_attention(nn.Module):
    def __init__(self, n_channels, n_classes, initial_feature_map=64, usa_cuda=True):
        ...
        # decoder
        self.up1 = Up2(self.initial_feature_map*16, self.initial_feature_map*8)
        self.up1.apply(self.weights_init)
        self.up2 = Up2(self.initial_feature_map*8, self.initial_feature_map*4)
        self.up2.apply(self.weights_init)
        self.up3 = Up2(self.initial_feature_map*4, self.initial_feature_map*2)
        self.up3.apply(self.weights_init)
        self.up4 = Up2(self.initial_feature_map*2, self.initial_feature_map)
        self.up4.apply(self.weights_init)
        self.outc = OutConv2(self.initial_feature_map, n_classes)
        self.outc.apply(self.weights_init)
...
class UNet_fourier1_2(nn.Module):
    def __init__(self, n_channels, n_classes, initial_feature_map=64, usa_cuda=True):
        ...
        # Decoder Binary
        self.up1_bin = Up2(self.initial_feature_map*16,
                          self.initial_feature_map*8)
        self.up1_bin.apply(self.weights_init)
        self.up2_bin = Up2(self.initial_feature_map*8,
                          self.initial_feature_map*4)
        self.up2_bin.apply(self.weights_init)
        self.up3_bin = Up2(self.initial_feature_map*4,
        ...
        self.up1_fouter1 = Up2(self.initial_feature_map*16,
                              self.initial_feature_map*8)
        self.up1_fouter1.apply(self.weights_init)
        self.up2_fouter1 = Up2(self.initial_feature_map*8,
                              self.initial_feature_map*4)
        self.up2_fouter1.apply(self.weights_init)
        self.up3_fouter1 = Up2(self.initial_feature_map*4,
                              self.initial_feature_map*2)
        self.up3_fouter1.apply(self.weights_init)
        self.up4_fouter1 = Up2(self.initial_feature_map *
                              2, self.initial_feature_map)
        self.up4_fouter1.apply(self.weights_init)
        self.outc_fouter1 = OutConv2(self.initial_feature_map, n_classes)
        self.outc_fouter1.apply(self.weights_init)


        # Decoder Fourier Outer 2
        self.up1_fouter2 = Up2(self.initial_feature_map*16,
                          self.initial_feature_map*8)
        self.up1_fouter2.apply(self.weights_init)
        self.up2_fouter2 = Up2(self.initial_feature_map*8,
                          self.initial_feature_map*4)
        self.up2_fouter2.apply(self.weights_init)
        self.up3_fouter2 = Up2(self.initial_feature_map*4,
                          self.initial_feature_map*2)
        self.up3_fouter2.apply(self.weights_init)
        self.up4_fouter2 = Up2(self.initial_feature_map *
                              2, self.initial_feature_map)
        self.up4_fouter2.apply(self.weights_init)
        self.outc_fouter2 = OutConv2(self.initial_feature_map, n_classes)
...
class UNet_fourier1(nn.Module):
    ...
    def forward(self, x):
        ...
        x_bin = self.up1_bin(x5, x4)  # 1024(x5) --> ConvT --> 512 concat with x4(512) --> 1024 --> DoubleConv2 --> 512 
        x_bin = self.up2_bin(x_bin, x3)
        x_bin = self.up3_bin(x_bin, x2)
        x_bin = self.up4_bin(x_bin, x1)
        logits_bin = self.outc_bin(x_bin)
