...
class CRIA(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        """print(torch.max(gt), torch.min(gt), torch.max(img), torch.min(img), img.shape, gt.shape)
...
def test_model(model, loss_type='mse'):
    ...
    for inputs, labels, names in tqdm(dataloaders['val']):
        ...
        for i,out in enumerate(outputs):
            ...
            #np_out = np_out * 255
            img = Image.fromarray(np_out[0]*255).convert('RGB')
            print(np.max(np_out), np.min(np_out), np_out.shape)
...
model = train_model(model, optimizer_ft, exp_lr_scheduler, patience = 30, num_epochs=30, loss_type=args.loss_type)
