...
class Results:
    ...
    def calculate_metrics(
        ...
        if mse:
            ...
            plt.clf()
        if acc:
            self.acc = round((tp + tn) / (tp + tn + fp + fn), 3)
        ...
        if dice_score:
            self.dice_score = round((2 * tp) / (fp + fn + (2 * tp)), 3)
            f.write("Dice Score:" + str(self.dice_score) + "\n")
            dice_based_image = round(sum(self.dice_list) / len(self.dice_list), 3)
            f.write(
                "Dice Score based on individual images:" + str(dice_based_image) + "\n"
            )
        if iou_score:
            iou_based_image = round(sum(self.iou_list) / len(self.iou_list), 3)
            f.write(
                "IOU Score based on individual images:" + str(iou_based_image) + "\n"
            )
        if fdr:
...
def post_process_reg(pred_dist_map):
    ...
    pred_dist_map = torch.sub(
        pred_dist_map, amin.reshape((amin.shape[0], amin.shape[1], 1, 1))
    )
...
def test_single(model, device, input_size, image_list, output_save_dir):
    ...
    for img_path in tqdm(image_list):
        ...
        img_org = (
            cv2.resize(cv2.imread(img_path, cv2.IMREAD_ANYDEPTH), input_size) / 255
        )
        ...
        pred_bin_img = np.array(pred_bin * 255, np.uint8)  #  * 255

        results.binary_metrics(gt_binary_mask, pred_bin)

        img8_8bit = (img_org * 255).astype("uint8")  # img_org / 256
        ...
        save_img_bin = np.hstack([img8_8bit, seperater, mask, seperater, pred_bin_img])
        cv2.imwrite(
            os.path.join(results_save_dir_images, image_name + ".png"), save_img_bin
        )
...
def test_multitask(model, device, input_size, image_list, output_save_dir):
    ...
    for img_path in tqdm(image_list):
        ...
        img_org = cv2.resize(cv2.imread(img_path, cv2.IMREAD_ANYDEPTH), input_size)
...
if __name__ == "__main__":
    ...
    model_path = "exp1_single_task_16\models\last_epoch.pt"
