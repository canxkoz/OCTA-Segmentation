...
from Model import UNet, UNet_multitask, UNet_attention, UNet_fourier1,UNet_Drop
...
def train_model_multi_task(
    ...
    params = [p for p in model.parameters()] + [log_var_task1] + [log_var_task2]
        ...
        for phase in ["train", "val"]:
            ...
            for inputs, label_mask, label_dist in tqdm(dataloaders[phase]):
                ...
                with torch.set_grad_enabled(phase == "train"):
                    ...
                    loss1 = calc_loss(output_mask, label_mask, loss_type=loss_function)

                    # regression
                    amin = torch.amin(output_dist, dim=(-2, -1))
                    output_dist = torch.sub(
                        output_dist, amin.reshape((amin.shape[0], amin.shape[1], 1, 1))
                    )
                    loss2 = calc_loss(output_dist, label_dist, loss_type="mse")

                    loss = loss_combiner([loss1, loss2], [log_var_task1, log_var_task2])
                    ...
                    if phase == "train":
                        ...
                        epoch_loss += loss.item()
                        val_loss += calc_loss(
                            output_mask, label_mask, loss_type=accuracy_metric
                        )
    ...
    plot_loss_functions(output_save_dir, train_loss_list, val_loss_list, "total")
    plot_loss_functions(output_save_dir, train_loss_list_1, val_loss_list_1, "bce")
    plot_loss_functions(output_save_dir, train_loss_list_2, val_loss_list_2, "mse")
...
def main(cfg):
    ...
    dropout_prob = cfg["model_config"]["drop_out_rate"]
    class_weights = cfg["model_config"]["class_weights"]
    ...
    class_loss_function = cfg["train_config"]["class_loss"]
    reg_loss_function =  cfg["train_config"]["reg_loss"]
    accuracy_metric = cfg["train_config"]["val_accuracy"]
    ...
    if model_type == "single_dropout":
        train_dataset = Data_Binary(train_path, ch, input_size=input_size)
        val_dataset = Data_Binary(val_path, ch, input_size=input_size)
        
        print(dropout_prob)

        model = UNet_Drop(ch, num_class, initial_filter_size, use_cuda, dropout=True, dropout_p = dropout_prob[0])
        model.apply(weights_init)

    elif model_type == "multi_alpha":
        train_dataset = Data_Reg_Binary(train_path, ch, input_size=input_size)
        ...
        train_dataset = Data_Reg_Fourier1(train_path, ch, input_size=input_size)
