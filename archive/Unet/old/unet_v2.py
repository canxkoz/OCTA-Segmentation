...
run = 'mse'
reshape_size = 512
model_name = 'UNet_fold{}_run_{}'.format(fold, run)
...
dataPath = 'drive/MyDrive/data/ROSE-2'
savePathPrefix = './'
...
def train_model(model, optimizer, scheduler, patience, loss_type, num_epochs=25):
...
#                 scheduler.step()
            ...
            for inputs, labels, names in tqdm(dataloaders[phase]):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    loss = calc_loss(outputs, labels, metrics, loss_type)
            ...
            if phase == 'val' and epoch_loss <= best_loss:
                ...
                best_model_wts = copy.deepcopy(model.state_dict())
                torch.save(best_model_wts, 'drive/MyDrive/data/models/' + model_name)
...
def test_model(model, loss_type, save_dir):
...
loss_type = 'mse'
model = train_model(model, optimizer_ft, exp_lr_scheduler, patience = 30, num_epochs=1, loss_type=loss_type)

output_save_dir = f'drive/MyDrive/outputs/{loss_type}/'
#model.load_state_dict(torch.load('./models/UNet_fold4_run_mse.pt'))
test_model(model, loss_type, output_save_dir)