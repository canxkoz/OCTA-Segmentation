...
def visual_results(loss_arr, auc_arr, acc_arr, sen_arr, fdr_arr, spe_arr, kappa_arr, gmean_arr, iou_arr, dice_arr, writer, epoch, flag=""):
...
def val_first_stage(best_thin, best_thick, best_fusion, writer, dataloader, net,
    ...
    thin_auc = round(visual_results(loss_dct["thin"], auc_dct["thin"], acc_dct["thin"], sen_dct["thin"], fdr_dct["thin"], spe_dct["thin"], kappa_dct["thin"], gmean_dct["thin"], iou_dct["thin"], dice_dct["thin"], writer, epoch, flag="_thin") + 1e-12, 4)
    thick_auc = round(visual_results(loss_dct["thick"], auc_dct["thick"], acc_dct["thick"],
                      ...
                      gmean_dct["thick"], iou_dct["thick"], dice_dct["thick"], writer, epoch,
                      flag="_thick") + 1e-12, 4)
    fusion_auc = round(visual_results(loss_dct["fusion"], auc_dct["fusion"], acc_dct["fusion"],
                       ...
                       gmean_dct["fusion"], iou_dct["fusion"], dice_dct["fusion"], writer,
...
def val_second_stage(best_fusion, writer, dataloader, first_net_thick, first_net_thin, fusion_net, criterion, device, save_epoch_freq, models_dir, results_dir, epoch, num_epochs=100):
    fusion_net.eval()
    ...
    fusion_auc = round(visual_results(loss_arr, auc_arr, acc_arr, sen_arr, fdr_arr, spe_arr, kappa_arr, gmean_arr, iou_arr, dice_arr, writer, epoch) + 1e-12, 4)
