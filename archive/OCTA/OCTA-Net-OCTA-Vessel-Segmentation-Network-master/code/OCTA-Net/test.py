...
def get_results(loss_lst, auc_lst, acc_lst, sen_lst, fdr_lst, spe_lst, kappa_lst, gmean_lst, iou_lst, dice_lst, dataloader, results_dir, criterion, pred, gt, mask_arr=None, isSave=True):
    ...
    pink_pred_img = np.copy(thresh_pred_img)
    pink_pred_img = np.expand_dims(pink_pred_img, 2)
    pink_pred_img = np.repeat(pink_pred_img, 3, 2)
    pink = (255,0,255)
    ...
    pink_pred_img[white_pixels] = pink

    merged_img = np.zeros((gt_img.shape[0], gt_img.shape[1] * 2, 3))
    for i in range(3):
        merged_img[:, 0:gt_img.shape[1], i] = gt_img
    merged_img[:, gt_img.shape[1]:, :] = pink_pred_img

    if isSave:
        mkdir(results_dir + "/Thresh")
        mkdir(results_dir + "/noThresh")
        mkdir(results_dir + "/Comparison")
        cv2.imwrite(results_dir + "/noThresh/" + imgName, pred_img)
        cv2.imwrite(results_dir + "/Thresh/" + imgName, thresh_pred_img)
        cv2.imwrite(results_dir + "/Comparison/" + imgName, merged_img)
