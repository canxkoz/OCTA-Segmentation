...
if isTraining:  # train
    ...
    for epoch in range(args.second_epochs):
        ...
        print('-'*10)
        second_net = train_second_stage(writer, train_dataloader, first_net_thick, first_net_thin, second_net, second_optim, args.init_lr, fusion_criterion, device, args.power, epoch+300, args.second_epochs+300)
        if (epoch + 1) % args.val_epoch_freq == 0 or epoch == args.second_epochs - 1:
            second_net, best_fusion = val_second_stage(best_fusion, writer, val_dataloader,first_net_thick, first_net_thin, second_net,
